...
fn readArgBinary(reader: anytype) !Arg {
    ...
    return switch (arg_type) {
        ...
        .indirect1 => .{ .indirect1 = try readIndirect(reader, u8) },
        .indirect2 => .{ .indirect2 = try readIndirect(reader, u16) },
        .indirect4 => .{ .indirect4 = try readIndirect(reader, u32) },
        .level_text_offset => .{ .level_text_offset = try reader.readIntLittle(u16) },
        .mem_address => .{ .mem_address = try reader.readIntLittle(u16) },
    };
}

fn readIndirect(reader: anytype, val_type: type) !Indirect {
    const address = try reader.readIntLittle(u16);
    const value = switch (val_type) {
        u8, u16, u32 => |T| try reader.readIntLittle(T),
        else => @compileError("readIndirect val_type must be u8, u16, or u32"),
    };
...
fn getScalarValue(s: Scalar) u32 {
    return switch (s) {
        .immediate => |v| v,
        .indirect1, .indirect2, .indirect4 => |i| i.value,
    };
}
