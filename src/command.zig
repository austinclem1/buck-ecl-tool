...
pub const CommandParser = struct {
    ...
    const VarMap = struct {
        ...
        pub fn init(allocator: std.mem.Allocator) !VarMap {
            var map = std.AutoHashMap(Key, []const u8).init(allocator);
            errdefer map.deinit();

            var names_arena = std.heap.ArenaAllocator.init(allocator);
            errdefer names_arena.deinit();

            for (default_global_vars) |v| {
                const key = Key{
                    .address = v.address,
                    .size = v.size,
                };
                try map.put(key, v.name);
            }

            return .{
                .map = map,
                .names_arena = names_arena,
            };
        }
        ...
        pub fn contains(self: *const VarMap, key: Key) bool {
            return self.map.contains(key);
        }

        pub fn get(self: *const VarMap, key: Key) ?[]const u8 {
        ...
        pub fn putWithAutoGeneratedName(self: *VarMap, key: Key) !void {
            const allocator = self.names_arena.allocator();

            const size_letter = key.size.getLetter();
            const name = try std.fmt.allocPrint(allocator, "{c}var_{x}", .{ size_letter, key.address });
            errdefer allocator.free(name);

            try self.map.put(key, name);
        }

        const default_global_vars = [_]Var{
            .{ .address = 0x979b, .size = .byte, .name = "window_graphic" },
            .{ .address = 0x97ad, .size = .byte, .name = "land_type" },
            ...
            .{ .address = 0x9af6, .size = .byte, .name = "player_y" },
            .{ .address = 0x9af7, .size = .byte, .name = "player_x" },
            .{ .address = 0x9af9, .size = .byte, .name = "player_room_id" },
            .{ .address = 0x9afa, .size = .byte, .name = "player_dir" },
            .{ .address = 0x9bf6, .size = .byte, .name = "selected_character_status" },
            .{ .address = 0x9e08, .size = .byte, .name = "curdisk" },
        };
    };
    ...
    pub fn init(allocator: std.mem.Allocator, script_bytes: []const u8, text_bytes: []const u8) !CommandParser {
        ...
        var vars = try VarMap.init(allocator);
        errdefer vars.deinit();

        var initialized_data_references = std.ArrayList(u16).init(allocator);
        errdefer initialized_data_references.deinit();
    ...
    pub fn parseEcl(self: *CommandParser) !void {
        ...
        if (self.initialized_data_references.items.len > 0) {
            std.debug.assert(self.initialized_data_references.items[0] > self.highest_known_command_address);
        }
        for (0..self.initialized_data_references.items.len) |i| {
            if (i + 1 >= self.initialized_data_references.items.len) break;

            const start_address = self.initialized_data_references.items[i];
            const end_address = self.initialized_data_references.items[i + 1];

            var buf: [32]u8 = undefined;
            const name = try std.fmt.bufPrint(&buf, "init_data{d}", .{i});

            try self.vars.put(.{ .address = start_address, .size = .byte }, name);

            const allocated_name = self.vars.get(.{ .address = start_address, .size = .byte }).?;
            try self.initialized_data_segments.append(.{
                .name = allocated_name,
                .bytes = self.script[start_address - ecl_base .. end_address - ecl_base],
            });
        }
    }
    ...
    fn trackArgIfVar(self: *CommandParser, arg: Arg) !void {
        ...
        if (self.vars.contains(key)) return;

        if (address >= ecl_scratch_space_start and address < ecl_scratch_space_end) {
            // this var references the ecl scratch space
            const scratch_offset = address - ecl_scratch_space_start;
            const size_letter = size.getLetter();
            var buf: [32]u8 = undefined;
            const name = try std.fmt.bufPrint(&buf, "scratch[{x}]{c}", .{ scratch_offset, size_letter });
            try self.vars.put(key, name);
        } else if (address >= ecl_base and address < ecl_base + self.script.len) {
            // must be a use of pre initialized data
            try self.vars.put(key, "");
            std.debug.assert(size == .byte);
            try self.initialized_data_references.append(address);
        } else {
            try self.vars.putWithAutoGeneratedName(key);
        }
    }
    ...
    const Arg = union(enum) {
        ...
        fn maybeGetVarSize(arg: Arg) ?Var.Size {
            return switch (arg) {
                .indirect1 => .byte,
                .indirect2 => .word,
                .indirect4 => .dword,
